...
openssl req -x509 -new -nodes -key ca-key.pem -days 10000 -out ca.pem -subj "/O=system:nodes CN=kube-ca"
...
openssl req -new -key apiserver-key.pem -out apiserver.csr -subj "/O=system:nodes, CN=system:node:${MASTER_HOST_IP}" -config openssl.cnf
...
WORKER_IP=${i} openssl req -new -key ${i}-worker-key.pem -out ${i}-worker.csr -subj "/O=system:nodes, CN=system:node:${i}" -config ../template/worker-openssl.cnf
...
openssl req -new -key admin-key.pem -out admin.csr -subj "/O=cluster-admin, CN=kube-admin"
openssl x509 -req -in admin.csr -CA ca.pem -CAkey ca-key.pem -CAcreateserial -out admin.pem -days 365

#create demouser certs
openssl genrsa -out demouser-key.pem 2048
openssl req -new -key demouser-key.pem -out demouser.csr -subj "/O=cluster-admin, CN=demouser"
