...
openssl req -new -key apiserver-key.pem -out apiserver.csr -subj "/O=system:nodes, CN=${MASTER_HOST_IP}" -config openssl.cnf
...
WORKER_IP=${i} openssl req -new -key ${i}-worker-key.pem -out ${i}-worker.csr -subj "/O=system:nodes, CN=${i}" -config ../template/worker-openssl.cnf
